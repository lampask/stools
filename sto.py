...
while(True):
    ...
    for cont in contours:
        if cv2.contourArea(cont) <= TARGET_MINIMAL_AREA:
            ...
            M = cv2.moments(cont)
            target_points.append([M['m10']/M['m00'],M['m01']/M['m00']])
    if len(target_points) == 3:
        ...
        operator_points.sort(key=lambda x: x[0])

        # CONTROLS
        if not mining:
            if abs(operator_points[0][0]-target_points[0][0]) >= TURNING_CONTROL_OFFSET or abs(operator_points[0][1]-target_points[0][1]) >= TURNING_CONTROL_OFFSET:
                if operator_points[0][1] > target_points[0][1]:
                    send_keys('{RIGHT}')
                else:
                    send_keys('{LEFT}')
            elif frames >= MINING_ZOOM_FRAME_DROP:
                dirs = [0, 0]
                for p1, p2 in zip(operator_points, target_points):
